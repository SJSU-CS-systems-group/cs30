...
| --- | --- | --- |
| `backend` | `CourseServiceTest`, `CodeServiceTest`, `LabServiceTest`, `ProblemServiceTest`, `CourseAccessServiceTest`, `LabHealthServiceTest`, `StudentIdentityServiceTest`, `ApiTokenStoreTest`, `CanvasSyncServiceTest`, `Cs30CourseMatchTest` | Service-layer logic; `CourseAccessServiceTest` pins who counts as a member and that only the TA escapes the lab window; the Canvas one also reads a submission tree from a temp dir, refuses a problem name that is not in the lab, and leaves the TA off the roster; the course-match one pins down how a course fragment resolves and what a miss lists |
...
| `cli` | `CanvasCliSpecTest`, `CanvasAssignmentNameTest`, `CanvasCommentTest`, `CanvasCourseMatchTest`, `CanvasMirrorTest` | The Canvas commands: option wiring, assignment naming, comment text, picking a Canvas course from a fragment, and the whole flow against a mocked server and a mocked Canvas |
