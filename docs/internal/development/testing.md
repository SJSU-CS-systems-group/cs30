...
| --- | --- | --- |
| `backend` | `CourseServiceTest`, `CodeServiceTest`, `LabServiceTest`, `ProblemServiceTest`, `CourseAccessServiceTest`, `LabHealthServiceTest`, `StudentIdentityServiceTest`, `ApiTokenStoreTest`, `CanvasSyncServiceTest` | Service-layer logic; `CourseAccessServiceTest` pins who counts as a member and that only the TA escapes the lab window; the Canvas one also reads a submission tree from a temp dir, refuses a problem name that is not in the lab, and leaves the TA off the roster |
