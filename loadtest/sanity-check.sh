...
#
# parseSubmit now refuses to report a verdict when (a) the run hit its wall timeout, (b) bt printed a
# Python traceback, or (c) fewer cases were graded than the problem has. That closes a real defect: a
# truncated grade used to be shown to the student as "Submitted: AC (2/2 passed)" on a 100-case problem.
#
# The danger is the mirror image — rejecting a submission that was actually complete. That would be worse
# than the bug being fixed, because it fails students who wrote correct code. Guard (c) is the one that can
