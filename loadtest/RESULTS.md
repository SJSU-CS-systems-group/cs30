...
|---|---|
| **Interactive problems** | **Problem 13** becomes unreliable on a server that has been under load for a long time. Cause not identified; no configuration change is known to prevent it. A restart clears it. |
...
**Interactive problems are not yet reliable, and the cause is not known.** If you assign one — where the
student's program and the checker exchange messages while both run — expect it to work on a freshly started
server and to become unreliable on a server that has been under load for a long time. On a clean host, 16
concurrent gradings of a 101-case interactive problem all completed in under 30 seconds. On a host that had
been running load tests for hours, the same command left one grading stuck indefinitely, and a repeat minutes
later had 9 of 16 quit early having graded almost nothing. A reboot restored it completely.

No configuration change is known to fix this. A server setting (`fs.pipe-user-pages-soft`) was believed to be
the fix and is not — the failure occurs identically with it at the default and raised 16x. Until the cause is
found, restart the server before a lab that uses an interactive problem. Non-interactive problems showed none
of this.
...
That difference is why only this problem is affected. When two programs have to exchange messages, each can
end up waiting for the other. The other 12 problems have nothing to wait for.

**The cause is not identified.** What follows is what was measured, and what was ruled out.

### What was seen

Grading this problem directly, 16 at a time, one container each:

| host condition | result |
|---|---|
| 1 at a time, idle host | all 101 cases, 16s |
| 16 at once, freshly rebooted host | **16 of 16 finished**, all 101 cases, under 30s |
| 16 at once, host that had run load tests for hours | 15 of 16 finished in ~120s; **1 never finished** — still at 0 cases after 300s |
| 16 at once, same host minutes later | **9 of 16 quit early**, having graded 0 to 18 cases |

Through the app, 100 students at once, on the loaded host: every submission failed, each taking about 60
seconds — the judge's wall timeout — and one student alone on an idle server failed the same way.

The pattern is host condition, not concurrency. On a clean host 16-way is only about twice as slow as a
single run and does not fail at all. Every failure above happened on a server that had been under load for
hours, and a reboot cleared them completely.

### What was ruled out

**A server setting was believed to be the fix. It is not.** `fs.pipe-user-pages-soft` limits how much memory
one user's pipes may hold, and the theory was that interactive grading exceeded it and got undersized
channels. Tested directly: the failure occurs identically with the limit at its 64 MB default and raised 16x
to 1 GB. Host pipe usage was low in both cases. An earlier result that appeared to prove otherwise compared
two runs an hour apart on a host that was drifting the whole time — not a controlled test.

**The grading tool's version is not implicated.** The sandbox image was unchanged across every run above.

### What a stalled grading looks like inside

Captured while still wedged: the grading worker was inside `subprocess.Popen.wait()` in the grading tool's
interactive module, waiting for a child process to exit. Its two child processes were both blocked reading
from pipes. The main thread was only waiting for that worker to finish. Those are symptoms of something
upstream, not the cause.

Two incidental facts worth knowing. The grading tool exits non-zero even on completely successful runs, so
exit code cannot be used to detect failure — count graded cases instead. And a failure that takes exactly 60
seconds is the judge's wall timeout doing its job, not a measurement of anything.

### It already fails safely

The grading system gives up on any submission after one minute and shuts it down properly. Each of the 100
students got a clear error instead of a frozen page, and no student was told their correct code passed when
the grading had not finished. Every submission still reached git. Nothing leaked and nothing needed manual
restarting.

### What to do meanwhile

Restart the server before a lab that uses an interactive problem, and re-check that the problem grades. There
is no configuration change known to prevent this.
...
These numbers are recorded because they were gathered, not because they explain anything. A mechanism was
inferred from them — that exceeding the pipe budget yields undersized pipes and a deadlock — and testing
disproved it: the failure occurs identically with `fs.pipe-user-pages-soft` at 16384 and at 262144, and
measured host pipe usage stayed low throughout. The `EPERM` from `F_SETPIPE_SZ`
(`PermissionError: [Errno 1] Operation not permitted`) was seen once and is unexplained; it did not recur in
any later run at either setting.
...
| the interactive concurrency runs (16-of-16 clean, 1 stuck, 9 early exits) | `debug-interactive-hang.sh`, which writes only to `/tmp` | not retained; re-runnable, but results depend on how long the host has been under load |
...
```bash
./debug-interactive-hang.sh 16 300            # grace must exceed ~120s or working runs look frozen
```

The grace period matters. At 16-way a healthy batch needs about 120 seconds on a loaded host, so a 90-second
limit reports working containers as stuck — that mistake produced an "everything froze" reading twice. Compare
completed-case counts, not liveness: a container that has graded 101 cases is done regardless of how it exits.
