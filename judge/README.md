...
| `problems_dir` | `problems` | where problem packages live (one dir per `problem_id`) |
...
|---|---|---|---|
| `problem_id` | string | yes | Problem directory name, e.g. `"arrayshift"`. No `/`, `\`, or leading `.`. |
...
| Field | Type | Required | Description |
|---|---|---|---|
| `problem_id` | string | yes | Problem directory name. |
...
curl -s -X POST http://localhost:8000/submit \
  ...
  -d '{"problem_id":"arrayshift","language":"python","source":"print(input())\n"}'

# Run on a custom input
curl -s -X POST http://localhost:8000/run \
  -H 'Content-Type: application/json' \
  -d '{"problem_id":"arrayshift","language":"python","stdin":"6 2\n2 2 4 4 0 0\n","source":"print(input())\n"}'
```
